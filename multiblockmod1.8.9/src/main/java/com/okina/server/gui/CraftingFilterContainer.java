...
public class CraftingFilterContainer extends Container {
		...
		if(true){
			...
			if((command == 0 || command == 1) && (mouseButton == 0 || mouseButton == 1)){
				...
				if(slotIndex == -999){
					...
					clickedSlot = inventorySlots.get(slotIndex);
				...
				if(clickedStack != null && (clickedSlot == null || !clickedSlot.getHasStack() || !clickedSlot.canTakeStack(player))){
					...
					for (int i2 = 0; i2 < 2; ++i2){
						for (int j2 = i1; j2 >= 0 && j2 < inventorySlots.size() && clickedStack.stackSize < clickedStack.getMaxStackSize(); j2 += givingStackSize){
							...
							if(slot3.getHasStack() && canAddItemToSlot(slot3, clickedStack, true) && slot3.canTakeStack(player) && func_94530_a(clickedStack, slot3) && (i2 != 0 || slot3.getStack().stackSize != slot3.getStack().getMaxStackSize())){
	...
	public boolean func_94530_a(ItemStack itemStack, Slot slot) {
		...
		return slot.inventory.isItemValidForSlot(slot.slotNumber, itemStack) && super.canMergeSlot(itemStack, slot);
	}
