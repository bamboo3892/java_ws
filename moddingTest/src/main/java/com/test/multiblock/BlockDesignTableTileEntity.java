...
public class BlockDesignTableTileEntity extends TileEntity implements IInventory{
	...
	public ItemStack getStackInSlot(int slot) {
		return slot == 0 ? recipeSheet : null;
	}
	@Override
	public ItemStack decrStackSize(int slot, int maxAmount) {
		if(slot == 0){
			ItemStack removed = recipeSheet;
			recipeSheet = null;
			this.markDirty();
			return removed;
		}
	...
	public boolean isItemValidForSlot(int slot, ItemStack itemstack) {
		return slot == 0 && itemstack != null && itemstack.getItem() == TestCore.craftingFilter;
	}
	...
	public void readFromNBT(NBTTagCompound tag) {
		...
		this.recipeSheet = ItemStack.loadItemStackFromNBT(itemTag);
	...
	public void writeToNBT(NBTTagCompound tag) {
		...
		if(recipeSheet != null){
			...
			this.recipeSheet.writeToNBT(itemTag1);
