	...
	public ConstructFilterContainer(IInventory player, FilterInventory filter) {
		...
		for (int col = 0; col < 3; ++col){
			for (int row = 0; row < 3; ++row){
				this.addSlotToContainer(new GhostSlot(filter, row + col * 3, 62 + row * 18, 17 + col * 18));
			}
		}
		for (int col = 0; col < 3; ++col){
			for (int row = 0; row < 9; ++row){
				this.addSlotToContainer(new Slot(player, row + col * 9 + 9, 8 + row * 18, 84 + col * 18));
			}
		}
		for (int row = 0; row < 9; ++row){
