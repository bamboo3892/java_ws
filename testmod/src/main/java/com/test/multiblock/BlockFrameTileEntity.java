...
public class BlockFrameTileEntity extends TileEntity {
			...
			for (int i1 = (p1.x < p2.x ? p1.x : p2.x); i1 <= (p1.x < p2.x ? p2.x : p1.x); i1++){
				for (int i2 = (p1.y < p2.y ? p1.y : p2.y); i2 <= (p1.y < p2.y ? p2.y : p1.y); i2++){
					for (int i3 = (p1.z < p2.z ? p1.z : p2.z); i3 <= (p1.z < p2.z ? p2.z : p1.z); i3++){
						if(worldObj.getTileEntity(i1, i2, i3) instanceof ConstructInterfaceTileEntity){
							if(count){
						...
						}else if(worldObj.getTileEntity(i1, i2, i3) != null){
			...
			for (int i2 = (p1.y < p2.y ? p1.y : p2.y) + 1; i2 < (p1.y < p2.y ? p2.y : p1.y); i2++){
				for (int i3 = (p1.z < p2.z ? p1.z : p2.z) + 1; i3 < (p1.z < p2.z ? p2.z : p1.z); i3++){
					if(worldObj.getTileEntity(i1, i2, i3) instanceof ConstructInterfaceTileEntity){
						...
						return false;
					}else if(worldObj.getTileEntity(i1, i2, i3) instanceof BlockPipeTileEntity || worldObj.getTileEntity(i1, i2, i3) instanceof ConstructBaseTileEntity){
	...
	private void writeNBTToItemStack(ItemStack itemStack, RectangularSolid solid, ArrayList<Position> interfaceList) {
		...
		for (int index = 0; index < solid.getIndexSize(); index++){
			...
			if(worldObj.getTileEntity(p.x, p.y, p.z) instanceof ConstructBaseTileEntity){
				ConstructBaseTileEntity base = (ConstructBaseTileEntity) worldObj.getTileEntity(p.x, p.y, p.z);
		...
		for (Position p : interfaceList){
			if(worldObj.getTileEntity(p.x, p.y, p.z) instanceof ConstructInterfaceTileEntity){
				ConstructInterfaceTileEntity inter = (ConstructInterfaceTileEntity) worldObj.getTileEntity(p.x, p.y, p.z);
