...
public class ConstructInterfaceTileEntity extends ConstructBaseTileEntity implements IConstructInventory, ILinkConnectionUser {
	...
	public boolean canInsertItem(int slotIndex, ItemStack itemStack, int side) {
		if(connection != null && connection.getTile() instanceof ISidedInventory){
			int io = ((ConstructBaseTileEntity) this.connection.getTile()).flagIO[side];
			((ConstructBaseTileEntity) this.connection.getTile()).flagIO[side] = 0;
			boolean bbb = this.connection.getTile().canInsertItem(slotIndex, itemStack, side);
			((ConstructBaseTileEntity) this.connection.getTile()).flagIO[side] = io;
			return bbb;
		}else if(connection != null && connection.getTile() instanceof IInventory){
			return true;
		}else{
	...
	public boolean canExtractItem(int slotIndex, ItemStack itemStack, int side) {
		if(connection != null && connection.getTile() instanceof ISidedInventory){
			int io = ((ConstructBaseTileEntity) this.connection.getTile()).flagIO[side];
			((ConstructBaseTileEntity) this.connection.getTile()).flagIO[side] = 1;
			boolean bbb = this.connection.getTile().canExtractItem(slotIndex, itemStack, side);
			((ConstructBaseTileEntity) this.connection.getTile()).flagIO[side] = io;
			return bbb;
		}else if(connection != null && connection.getTile() instanceof IInventory){
			return true;
